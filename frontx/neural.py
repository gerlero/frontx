...
class _PINN(eqx.Module):
    ...
    _net: eqx.nn.MLP = eqx.field(default_factory = lambda: eqx.nn.MLP(
        "scalar",
        "scalar",
        8,
        3,
    ...
    def physics_loss(
        ...
        *,
        ...
        #residuals, _ = jax.lax.approx_min_k(jnp.abs(residuals), 498)

        #residuals = jnp.where(jnp.abs(residuals) > 1, jnp.nan, residuals)
        ...
        residuals_ = jnp.where(jnp.abs(residuals) > delta, jnp.nan, residuals)

        residuals = delta*jnp.tanh(residuals / delta)

        return (
            jnp.mean(residuals ** 2)
            + self(1.0) ** 2
            + jax.grad(self)(1.0) ** 2
        ), jnp.nanmean(residuals_ ** 2) + self(1.0) ** 2 + jax.grad(self)(1.0) ** 2
...
def fit(  # noqa: PLR0913
    ...
    *,
    ...
    initial_data_loss = net.data_loss(
        x_data, y_data, y_sigma=y_sigma)
    ...
    def loss(trainable_net: _PINN, *, step: int, x: jax.Array) -> jax.Array:
        net = eqx.combine(trainable_net, static_net)

        physics_loss, physics_loss_ = net.physics_loss(
            x,
            i=i,
            b=b,
            oi=oi,
        ...
        jax.debug.print(
            "step: {step}, data_loss: {data_loss}, physics_loss: {physics_loss}",
            step=step,
            data_loss=data_loss,
            physics_loss=physics_loss_,
        )

        lambda_ = initial_data_loss * 10**(-2 + step / 100_000)

        return data_loss + lambda_ * physics_loss, physics_loss_
    ...
    def adam_train_step(
        ...
        (_, physics_loss), grads = jax.value_and_grad(loss, has_aux=True)(trainable_net, step=step, x=x)
    ...
    def adam_train_steps(
        ...
        trainable_net, opt_state, physics_loss, step = jax.lax.fori_loop(
            0,
            100,
            lambda _, val: adam_train_step(*val, x=x),
            (trainable_net, opt_state, physics_loss, step),
        )
        return trainable_net, opt_state, physics_loss, step

    trainable_net, opt_state, physics_loss, step = jax.lax.while_loop(
        lambda val: (val[2] > 1e-5) & (val[3] < max_steps),
        lambda val: adam_train_steps(*val),
