...
Generates the figure comparing four models (Brooks and Corey, Van Genuchten,
...
def run(x_grid: Sequence[float] | None = None) -> None:
    ...
    d = LETd(
        L=0.004569, E=12930, T=1.505, Dwt=4.660e-4, theta_range=(0.019852, theta_s)
    )

    theta_bc = frontx.solve(bc, i=theta_i, b=theta_b)
    theta_vg = frontx.solve(vg, i=theta_i, b=theta_b)
    theta_xs = frontx.solve(xs, i=theta_i, b=theta_b)
    theta_d = frontx.solve(d, i=theta_i, b=theta_b)
